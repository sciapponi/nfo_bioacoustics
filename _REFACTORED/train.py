#!/usr/bin/env python3
"""Train a sample-rate-invariant, multi-task bioacoustic classifier.

Expected input layout:

    <root>/manifests/all_samples.csv
    <root>/manifests/all_classes.csv

The script builds a reduced N-class-per-dataset problem from the source manifest,
then writes clean split-specific manifests under the run directory:

    <run_dir>/manifests_reduced/train_samples.csv
    <run_dir>/manifests_reduced/val_samples.csv
    <run_dir>/manifests_reduced/test_samples.csv
    <run_dir>/manifests_reduced/classes.csv

Validation is used for model selection. Test is evaluated once at the end using
best.pt.
"""
...
from models.model import DATASET_NAMES, SRInvariantBioacousticClassifier
...
def split_key(row: dict[str, Any]) -> str:
    """Key used for deterministic derived splits.

    Prefer source_id so clips from the same source file remain in the same split
    when the manifest does not already provide val/test splits.
    """
...
def choose_augmented_sr(sr: int, min_factor: float, max_factor: float, round_to: int) -> tuple[int, float]:
...
def make_sr_view(
    ...
    for audio, sr in zip(audios, sample_rates):
        if augment:
            target_sr, _ = choose_augmented_sr(sr, min_factor, max_factor, round_to)
        else:
            target_sr = int(sr)
...
def sr_bucket_labels(sample_rates: Sequence[int], device: torch.device) -> torch.Tensor:
    edges = torch.tensor(
        [500, 4_000, 12_000, 20_000, 28_000, 44_000, 88_000],
        dtype=torch.float32,
        device=device,
    )
...
def sr_adversary_loss(outputs: dict[str, Any], sample_rates: Sequence[int], device: torch.device) -> torch.Tensor:
    sr_logits = outputs.get("sr_logits")
    if sr_logits is None:
        return torch.zeros((), device=device)
    labels = sr_bucket_labels(sample_rates, device)
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            out1 = model(view1, sr1, grl_lambda=args.grl_lambda, return_sr_logits=True)
            out2 = model(view2, sr2, grl_lambda=args.grl_lambda, return_sr_logits=True)
            ...
            adv_loss = 0.5 * (
                sr_adversary_loss(out1, sr1, device) + sr_adversary_loss(out2, sr2, device)
            )
...
def evaluate(
    ...
    for batch in loader:
        steps += 1
        view, sr = make_sr_view(batch, device, 1.0, 1.0, 0, augment=False)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train sample-rate-invariant reduced-class bioacoustic model.")
...
def main() -> int:
    ...
    run_dir = Path(args.run_dir).expanduser().resolve() if args.run_dir else root / "runs" / "sr_invariant_reduced"
    ...
    (run_dir / "test_metrics.json").write_text(
        json.dumps({"checkpoint": str(best_path), "epoch": loaded_epoch, "test": test_metrics}, indent=2),
        encoding="utf-8",
    )

    print(f"test_checkpoint={best_path} epoch={loaded_epoch:03d} {compact_metrics(test_metrics, prefix='test_')}", flush=True)
    return 0
